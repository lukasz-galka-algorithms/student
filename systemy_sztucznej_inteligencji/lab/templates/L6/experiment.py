...
class Experiment:
    """
    A class to run regression experiments using RegressionModel.
    """
    ...
    def run_experiment(self, steps):
        """
        Run a series of regression steps.
        ...
        steps : list of tuples
            ...
            - 'evaluate'          : test performance (MSE, R2)
            - 'cross_validate'    : k-fold CV performance
            ...
            if step == 'split':
                ...
                if self.reg is None:
                    ...
                    if self.X_test is None or self.y_test is None:
                        ...
                        perf = 0.0
                        # TODO - BLOCK END
                        results.append((step, {"performance": perf}))
                ...
                if self.reg is None:
                    ...
                    if cv is not None:
                        ...
                        cv_perf = 0.0
                        # TODO - BLOCK END
                    results.append((step, {"cv_performance": cv_perf}))
    ...
    def example_experiment_1(df, target_column):
        ...
        - Split data (test_size=0.25, random_state=42)
        - Train Linear Regression model
        - Evaluate performance on test data
        - Perform 5-fold cross-validation (cv=5)
        - Train Random Forest Regressor
        - Optimize hyperparameters using GridSearchCV (n_estimators=[50, 100], max_depth=[None, 5], cv=5, n_jobs=-1)
        - Evaluate performance on test data
        - Perform 5-fold cross-validation (cv=5)
        """
        # TODO - BLOCK START
    ...
    def example_experiment_2(df, target_column):
        ...
        - Split data (test_size=0.2, random_state=13)
        - Train K-Nearest Neighbors (default parameters)
        - Optimize hyperparameters using RandomizedSearchCV (n_neighbors=[3..31 step 2], weights=['uniform','distance'], p=[1,2], n_iter=20, cv=5, n_jobs=-1)
        - Evaluate performance on test data
        - Perform 5-fold cross-validation (cv=5)
        """
        # TODO - BLOCK START
